...
export default function FleetMatrix({ vesselIds, onVesselIdsChange, readOnly = false }: Props) {
  ...
  const fleetRange = useMemo(() => {
    ...
    for (const v of vessels) {
      end = Math.max(end, cy + v.holding_years - 1)
    }
  ...
  function removeVessel(id: string) {
    onVesselIdsChange(vesselIds.filter((x) => x !== id))
  }
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1 border border-ink-200 rounded-btn pl-2 pr-1 py-0.5 bg-white">
              ...
              <button
                ...
                onClick={() => createMut.mutate(addClass)}
        ...
        {isLoading ? (
          ...
          <div className="empty-state">
            ...
            <div className="text-xs text-ink-500 max-w-sm">
              {readOnly
                ? <>Go to the <Link to="/input" className="text-accent-600 hover:underline">Input page</Link> to add vessels to this fleet.</>
                : <>Use <b>+ Create vessel</b> for a new one or <b>+ From registry</b> to pull in an existing vessel.</>
              }
            </div>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="inst min-w-full">
              <thead>
                <tr>
                  <th className="sticky left-0 bg-white z-10">Vessel</th>
                  {fleetRange.years.map((y) => (
              ...
              <tbody>
                {vessels.map((v) => {
                  return (
                    <tr key={v.id}>
                      <td className="sticky left-0 bg-white z-10" style={{ borderLeft: `3px solid ${CLASS_COLORS[v.vessel_class] ?? '#9aa3b2'}` }}>
                        <div className="flex items-center gap-2">
                          <VesselThumb vessel={v} />
                          <div className="min-w-0">
                            <div className="text-[12px] font-medium text-ink-900 truncate">{v.name}</div>
                            <div className="text-[10px] text-ink-400">
                              {v.vessel_class} · purchased {v.purchase_date} · sale {currentYear() + v.holding_years - 1}
                            </div>
                            {readOnly ? (
                              <Link to="/input" className="text-[10px] text-accent-600 hover:underline">Edit in Input →</Link>
                            ) : (
                              <Link to={`/vessels#vessel-${v.id}`} className="text-[10px] text-accent-600 hover:underline">View in Vessels →</Link>
                            )}
                          </div>
                        </div>
                      </td>
                      {fleetRange.years.map((calYear, ci) => {
                        const oy = ownershipYearIdx(v, calYear)
                        if (oy == null) {
                          return <td key={ci} className="bg-ink-50/40"></td>
                        }
                      ...
                      {!readOnly && (
                        <td className="text-right">
                          <button
                            type="button"
                            className="text-ink-400 hover:text-danger text-[14px]"
